...
try {
    ...
    $sqlGetApproveStatus = "SELECT l_approve_status, l_approve_status2, l_approve_status3, l_approve_name, l_approve_name2, l_username
                            FROM leave_list
                            WHERE l_usercode = :userCode AND l_create_datetime = :createDate";
    ...
    // เงื่อนไขสำหรับ status = 5 (ไม่อนุมัติ)
    if ($status == '5') {
    ...
    foreach ($managers as $manager) {
        if (! empty($manager['e_user_id'])) {
            ...
            // ข้อความแจ้งเตือนขึ้นอยู่กับสถานะและระดับของผู้รับ
            if ($status == '4') {
                ...
                // ข้อความสำหรับผู้รับคนอื่นๆ
                else {
                    $message = "K." . $manager['e_username'] . "\n\n$proveName ไม่อนุมัติใบลาของ $empName\nประเภทการลา : $leaveType\nเหตุผลการลา : $leaveReason\nวันเวลาที่ลา : $leaveStartDate ถึง $leaveEndDate\nกรุณาเข้าสู่ระบบเพื่อดูรายละเอียด $sURL";
                    if ($leaveStatus == 'ยกเลิกใบลา') {
