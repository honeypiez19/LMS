...
<body>
    ...
    <div class="container">
        <form class="mt-4 mb-3 row" method="post" id="dateForm">
            ...
            <div class="col-auto">
                <?php
                                                                             // กำหนดวันที่เริ่มต้น                                                         // กำหนดวันที่เริ่มต้นและสิ้นสุด
                    $startDate = date('Y', strtotime('-1 year')) . "-12-01"; // 1 ธันวาคม ปีที่แล้ว
                    $endDate   = strtotime(date('Y') . "-11-30");            // 30 พฤศจิกายน ปีปัจจุบัน

                    // หากผู้ใช้เลือกช่วงวันที่ ก็จะใช้ค่าจาก $_POST
            ...
            <div class="col-auto">
                <input type="text" name="end_date" class="form-control" id="endDate"
                    value="<?php echo date("d-m-Y", $endDate); ?>">
            </div>
            <div class="col-auto">
        ...
        <div>
            <p>ช่วงวันที่ : <strong id="startDateText">
                    <?php
                        // ตรวจสอบค่าของ $startDate และแปลงเป็นวันที่ในรูปแบบที่ถูกต้อง
                        echo $startDate;
                    ?>
                </strong> ถึง <strong id="endDateText">
                    <?php
                        // ตรวจสอบค่าของ $endDate และแปลงเป็นวันที่ในรูปแบบที่ถูกต้อง
                        if ($endDate) {
                            $endDateObj = DateTime::createFromFormat('Y-m-d', date('Y-m-d', $endDate));
                            echo $endDateObj->format('d-m-Y'); // แสดงวันที่ในรูปแบบ dd-mm-yyyy
                        } else {
                            echo "วันที่ไม่ถูกต้อง"; // กรณีที่มีข้อผิดพลาดในการแปลง
                        }
                    ?>
                </strong>
            </p>
    ...
    <script>
    flatpickr("#startDate", {
        dateFormat: "d-m-Y", // รูปแบบที่ใช้ส่งข้อมูล
        // altFormat: "d-m-Y", // รูปแบบที่แสดงใน input field
        defaultDate: "<?php echo $startDate; ?>", // ใช้ค่าจาก PHP (แปลงเป็น Y-m-d)
        // onChange: function(selectedDates, dateStr, instance) {
        //     document.getElementById("startDateText").textContent = instance.formatDate(selectedDates[0],
        //         "d-m-Y");
        //     document.getElementById("dateForm").submit(); // ส่งฟอร์มหลังจากเลือกวันที่
        // }
    });

    // flatpickr สำหรับ endDate
    flatpickr("#endDate", {
        dateFormat: "Y-m-d", // รูปแบบที่ใช้ส่งข้อมูล
        altFormat: "d-m-Y", // รูปแบบที่แสดงใน input field
        defaultDate: "<?php echo date('Y-m-d', $endDate); ?>", // ใช้ค่าจาก PHP (แปลงเป็น Y-m-d)
        onChange: function(selectedDates, dateStr, instance) {
