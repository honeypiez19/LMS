                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="leaveForm" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="mt-3 row">
                                    <div class="col-12">
                                        <label for="file" class="form-label">ไฟล์แนบ (สูงสุด 3 ไฟล์: PNG, JPG,
                                            JPEG)</label>
                                        <input class="form-control" type="file" id="file" name="file" multiple
                                            accept="image/png, image/jpeg, image/jpg" />
                                        <small class="text-muted">สามารถเลือกไฟล์รูปภาพได้สูงสุด 3 ไฟล์</small>
        ...
        $(document).ready(function() {
            ...
            $('#leaveForm').submit(function(e) {
                ...
                $.ajax({
                    ...
                    หัวหน้าอนุมัติ: ${$('#approver option:selected').text()}<br>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response === 'double') {
                            ...
                            createDate.setHours(createDate.getHours() +
                                7);
                            var formattedDate = createDate.toISOString().slice(
                                    0, 19)
                                .replace('T', ' ');
                            fd.append('formattedDate', formattedDate);

                            if (!$('*[name="alertCheckDays"]').hasClass(
                                    'd-none')) {
                                Swal.fire({
                                    title: "ไม่สามารถลาได้",
                                    text: "ใช้สิทธิ์หมดแล้ว กรุณาเปลี่ยนประเภทการลา",
                            ...
                            if (urgentLeaveType == 'เลือกประเภทการลา') {
                                ...
                                var startDate = $('#urgentStartDate').val()
                                    .replace(
                                        /-/g,
                                        '');
                                var endDate = $('#urgentEndDate').val().replace(
                                    /-/g,
                                    '');
                                var startTime = $('#urgentStartTime')
                                    .val(); // เช่น "08:00"
                                var endTime = $('#urgentEndTime')
                                    .val(); // เช่น "17:00"
                                ...
                                var end = new Date(endDate.substring(0, 4),
                                    endDate
                                    .substring(4, 6) - 1, endDate
                                    .substring(6, 8), endTime.split(':')[0],
                                    endTime
                                    .split(':')[1]);

                                if (isNaN(start.getTime()) || isNaN(end
                                        .getTime())) {
                                    Swal.fire({
                                        title: "ข้อผิดพลาด",
                                        text: "วันที่หรือเวลาไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง",
                                ...
                                var fullDays = Math.floor(timeDiff / (1000 *
                                    3600 *
                                    8)); // จำนวนวันเต็ม
                                var remainingTimeInMs = timeDiff % (1000 *
                                    3600 *
                                    8); // มิลลิวินาทีที่เหลือจากวันเต็ม
                                var hoursDiff = Math.floor(remainingTimeInMs / (
                                    1000 *
                                    3600)); // จำนวนชั่วโมงที่เหลือ
                                var minutesDiff = Math.floor((
                                    remainingTimeInMs % (
                                        1000 *
                                        3600)) / (1000 *
                                    60)); // คำนวณนาทีที่เหลือ
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.error) {
                            ...
                            // เวลาที่สิ้นสุด
                            // 08:10
                            if (response.l_leave_end_time === "08:30:00" &&
                                response
