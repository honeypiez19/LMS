...
WHERE
    ...
    AND (
        -- แจ้งเตือนก่อนวันลา 3 วัน
        li.l_leave_start_date = CURDATE() + INTERVAL 3 DAY
        -- แจ้งเตือนหากวันลาเลยมา 3 วันแล้ว แต่ยังไม่มีการอนุมัติ
        OR (li.l_leave_start_date = CURDATE() - INTERVAL 3 DAY AND li.l_approve_status = 0)
    )
