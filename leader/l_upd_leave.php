...
// Case 15-16: status1 = 6, status2 = 6, status3 = 8 - แจ้งเตือน GM, admin
    ...
    // 16 status1 = 2, status2 = 6, status3 = 7, hrStatus = 0 - แจ้งเตือนหัวหน้า 1, GM
...
// กรณีอื่นๆ ที่ไม่เข้าเงื่อนไขด้านบน
    ...
    if ($stmt->execute()) {
        ...
        foreach ($notifiedUsers as $user) {
            // Skip notification if the user is the one who made the update
            if ($user['e_username'] === $userName) {
            ...
            // เก็บผลลัพธ์จาก API เพื่อตรวจสอบ
            $response = curl_exec($ch);
            $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);

            // ตรวจสอบผลลัพธ์
            if ($httpCode != 200) {
                error_log("LINE API Error: HTTP Code $httpCode, Response: $response");
            } else {
                error_log("LINE notification sent successfully to " . $user['e_username']);
            }
