...
// กำหนดค่าเริ่มต้น
    ...
    if ($urgentLeaveType == 5 && $leaveCondition) {
        ...
        , l_approve_name, l_approve_datetime, l_file2, l_file3, l_leave_id2)
        ...
        ,:filename2 ,:filename3,:leaveCondition)");
    ...
    if ($stmt->execute()) {
        // Change the query to fetch both user_id AND username
        ...
        $stmt->bindParam(':workplace', $workplace);
        $stmt->execute();
        $userList = $stmt->fetchAll(PDO::FETCH_ASSOC); // Fetch as associative array
