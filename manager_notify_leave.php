...
WHERE
    ...
    AND (
        -- แจ้งเตือนก่อนวันลา 2 วัน
        li.l_leave_start_date = CURDATE() + INTERVAL 2 DAY
        -- แจ้งเตือนหากวันลาเลยมา 2 วันแล้ว แต่ยังไม่มีการอนุมัติ
        OR (li.l_leave_start_date = CURDATE() - INTERVAL 2 DAY AND li.l_approve_status2 = 1)
    )
    AND li.l_level IN ('user','leader','chief')";
