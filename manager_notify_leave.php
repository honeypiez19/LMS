...
// คำนวณวันที่ควรแจ้งเตือน (2 วันทำงานก่อนและหลัง)
$today        = date('Y-m-d');
$notifyBefore = getNextWorkingDay($today, 2, $holidays);
$notifyAfter  = getPreviousWorkingDay($today, 2, $holidays);
...
WHERE
    ...
    AND (
        -- แจ้งเตือนล่วงหน้า 2 วันทำงาน (ไม่นับวันหยุด)
        li.l_leave_start_date = :notifyBefore
        -- แจ้งเตือนหากผ่านมา 2 วันทำงานแล้ว (ไม่นับวันหยุด) แต่ยังไม่มีการอนุมัติ
        OR (li.l_leave_start_date = :notifyAfter AND li.l_approve_status2 = 1)
    )
    AND li.l_level IN ('user','leader','subLeader','chief','admin')";
