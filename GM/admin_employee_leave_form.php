  ...
  border: none;">
    ...
    <div class="container-fluid">
        ...
        AND l_leave_status <> ";
                        ...
                        // คำสั่ง SQL สำหรับดึงข้อมูลทั้งหมดโดยไม่มีการ LIMIT
                        $dataSql = "SELECT * " . $baseSql . " ORDER BY l_create_datetime DESC";
...
`;
        ...
        $(document).on('click', '.cancel-btn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'a_ajax_delete_emp_leave.php',
                        type: 'POST',
                        data: {
                            l_usercode: usercode,
                            l_leave_id: leaveId,
                            ...
                            try {
                                ...
                                if (data.success) {
                                    ...
                                    Swal.fire({
                                        title: 'เกิดข้อผิดพลาด',
                                        text: data.message ||
                                            'ไม่สามารถยกเลิกใบลาได้',
                                        icon: 'error'
                                    });
                                }
                            } catch (e) {
                                // If not JSON, still attempt to reload
                                Swal.fire({
                                    title: 'ยกเลิกใบลาสำเร็จ',
                                    icon: 'success'
                                }).then(() => {
