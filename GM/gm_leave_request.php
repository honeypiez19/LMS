...
WHERE
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer d-flex justify-content-between">
                            <div>
                                <button type="button"
                                    class="btn btn-warning button-shadow btn-inquiry">ขอข้อมูลเพิ่มเติม</button>
                            </div>
                            <div>
    ...
    function loadPage(page) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#total-count-7").text(response.status_counts['7']);
                $("#total-count-8").text(response.status_counts['8']);
                $("#total-count-9").text(response.status_counts['9']);
    ...
    function attachTableEvents() {
            ...
            $('.modal-footer .btn-inquiry').off('click').on('click', function() {
                ...
                setTimeout(function() {
                    Swal.fire({
                        ...
                        input: 'textarea',
                        // inputLabel: 'ข้อความ',
                        inputPlaceholder: 'กรอกข้อความที่ต้องการสอบถามพนักงาน',
                        inputAttributes: {
                            'aria-label': 'กรอกข้อความที่ต้องการสอบถามพนักงาน',
                            'rows': 5
                        },
                        showCancelButton: true,
                        confirmButtonText: 'ส่งข้อความ',
                        cancelButtonText: 'ยกเลิก',
                        allowOutsideClick: false, // ไม่ให้ปิดเมื่อคลิกด้านนอก
                        backdrop: true, // แสดง backdrop แต่ไม่ให้คลิกได้
                        preConfirm: (inputValue) => {
                            if (!inputValue) {
                                Swal.showValidationMessage(
                                    'กรุณากรอกข้อความที่ต้องการสอบถาม');
                            }
                        }
                    }).then((result) => {
                        if (result.isConfirmed) {
                            const inquiryMessage = result.value; // รับค่าจาก input
                            ...
                            var status = '10'; // กำหนดสถานะเป็นรอตอบกลับจากพนักงาน
                            var userName = '<?php echo $userName; ?>';
                            var proveName = '<?php echo $name; ?>';
                            var level = '<?php echo $level; ?>';
                            var subDepart = '<?php echo $subDepart; ?>';
                            ...
                            $.ajax({
                                url: 'g_ajax_upd_status.php', // ใช้ไฟล์เดิม
                                method: 'POST',
                                data: {
                                    createDate: createDate,
                                    userCode: userCode,
                                    ...
                                    leaveStatus: leaveStatus,
                                    empName: empName,
                                    inquiryMessage: inquiryMessage, // ส่งข้อความสอบถาม
                                    level: level,
                                    subDepart: subDepart,
                                    subDepart2: subDepart2,
                                    subDepart3: subDepart3,
                                ...
                                success: function(response) {
                                    ...
                                    localStorage.setItem(
                                        'leaveSelectedMonth',
                                        selectedMonth);
                                    localStorage.setItem('leaveCodeSearch',
                                        codeSearch);
                                    localStorage.setItem('leaveNameSearch',
                                        nameSearch);

                                    Swal.fire({
                                        title: 'ส่งข้อความสำเร็จ !',
                                        text: 'ข้อความสอบถามได้ถูกส่งไปยังพนักงานเรียบร้อยแล้ว',
                                        icon: 'success',
                                        confirmButtonText: 'ตกลง',
                                        allowOutsideClick: false // ไม่ให้ปิดเมื่อคลิกด้านนอก
                                    }).then((result) => {
                            ...
                            $('#leaveModal').modal(
                                'show'); // ถ้ายกเลิก แสดง modal หลักอีกครั้ง
                        }
                    });
                }, 300);
            });
        ...
        $('.view-history').click(function() {
            ...
            $.ajax({
                url: 'g_ajax_get_leave_history.php',
                type: 'POST',
                data: {
                    userCode: userCode,
