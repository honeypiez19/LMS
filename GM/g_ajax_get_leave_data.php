...
if ($status !== 'all') {
    $sql .= " AND li.l_approve_status3 = :status";
}
...
// ดึงข้อมูลจำนวนรายการตามสถานะ (เพิ่มโค้ดส่วนนี้)
$statuses      = ['all', '7', '8', '9'];
$status_counts = [];
...
foreach ($statuses as $stat) {
    ...
    $status_sql = "SELECT COUNT(*) FROM leave_list li
                   INNER JOIN employees em ON li.l_usercode = em.e_usercode
                   WHERE li.l_approve_status IN (2,6)
                   AND li.l_approve_status2 IN (4,6)
                   AND li.l_approve_status3 <> 6
                   AND li.l_leave_id NOT IN (6, 7)
                   AND (
    ...
    if ($stat !== 'all') {
        $status_sql .= " AND li.l_approve_status3 = :status";
    }
...
$response = [
    ...
    'status_counts' => $status_counts, // เพิ่มข้อมูลจำนวนรายการตามสถานะ
];
